...
d<-d %>%
  group_by(variable, Year=cut(Date, "year"))%>%
  mutate(mean_val=mean(VAL.IND))

colnames(d)<-c("Date","Type","value","VAL.IND","Year","mean_val")
d$Year<-as.Date(d$Year)
ggplot(d, aes(x=Year,y=mean_val, col=Type)) + 
  geom_path()
```
...
total <- merge(ddata frameB,by=c("Type","Date")
