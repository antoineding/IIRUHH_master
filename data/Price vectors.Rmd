...
quintile_exposure$Date <- sub('-09-03', '-01-01', quintile_exposure$Date, fixed=TRUE)
...
ggplot(d, aes(fill=Type, y=Value, x=city_type)) + 
  ...
  labs(y="Share","Consumption share over time per city type", 
       title="Consumption share EA by city type and year",
...
ggplot(pq_city, aes(x=as.Date(Date, "%Y"), y=exposure_average, col=city_type)) + 
  geom_line()+
  labs(x="Date",y="Price index", title="5year price index by degree of urbanisation", caption="ECB SDW and Eurostat COICOP, Author's calculation")+
  theme_bw()
dev.off()
```
