...
p3<-ggplot(pq_quintile, aes(x=as.Date(Date, "%Y"),y=exposure_average, col=Quintile)) + 
  geom_line()+
...
p4<-ggplot(d, aes(fill=Type, y=Value, x=city_type)) + 
  geom_bar(position="stack", stat="identity")+
  facet_grid(~ Date)+
  theme_bw()+
...
png("Pindex_city.png")
p5<-ggplot(pq_city, aes(x=as.Date(Date, "%Y"), y=exposure_average, col=city_type)) + 
  geom_line()+
...
p6<-ggplot(d, aes(fill=Type, y=Value, x=CSP_WORK)) + 
  geom_bar(position="stack", stat="identity")+
  facet_grid(~ Date)+
  theme_bw()+
  labs(y="Share","Consumption share over time per city type", 
...
png("Pindex_csp.png")
p7<-ggplot(pq_csp, aes(x=as.Date(Date, "%Y"), y=exposure_average, col=CSP_WORK)) + 
  geom_line()+
  labs(x="Date",y="Price index", title="5year price index by activity status in France", caption="ECB SDW and Eurostat COICOP, Author's calculation")+
  theme_bw()
dev.off()
p7
```
...
ggplot(pq_age, aes(x=as.Date(Date, "%Y"), y=exposure_average, col=AGE_RANGE)) + 
  geom_line()+
  labs(x="Date",y="Price index", title="5year price index by age in France", caption="ECB SDW and Eurostat COICOP, Author's calculation")+
  theme_bw()
dev.off()
```
