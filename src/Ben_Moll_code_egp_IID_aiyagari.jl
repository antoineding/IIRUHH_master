...
#Equm iter 47, r = 0.05165967685123596, KL ratio: 5.0348284883204215 KL diff: 0.00036207376761243637%
# 54.166175 seconds (16.01 M allocations: 52.434 GiB, 14.52% gc time, 3.61% compilation time)
#Mean assets: 2.6374937399308496
#Fraction borrowing constrained: 0.02%
#10th Percentile: 0.6818937858385097
#50th Percentile: 1.7503525552311197
#90th Percentile: 5.393835534383105
#99th Percentile: 16.249131292736276
...
@time while iterKL<=maxiter_KL && abs(KLdiff)>tol_KL
    ...
    if iterKL==1
        ...
        for iy = 1:ny
            conguess[:,iy] = r.*agrid .+ wage.*ygrid[iy]
        end
    ...
    while iter <= max_iter && cdiff>tol_iter
        ...
        for iy = 1:ny
            ...
            ass1[:,iy] = (con1 .+ agrid .-wage.*ygrid[iy])./R
            ...
            con[:,iy] = R.*agrid .+ wage.*ygrid[iy] - sav[:,iy]
        end
    ...
    for iy = 1:ny
        savinterp[iy] = interpolate((1.001*agrid,), sav[:,iy], Gridded(Linear()))
    end
...
if MakePlots==1
    ...
    display(p6)
    savefig(p6, "Figures/Aiyagari_asset.png")
